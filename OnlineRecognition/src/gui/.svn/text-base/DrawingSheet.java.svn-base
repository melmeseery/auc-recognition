 ...
 * @author Mahi
 */
public class DrawingSheet extends JScrollPane  implements Observer{
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(DrawingSheet.class);
	boolean flagBoxDraw=false ;
	GuiPageModel StrokeData;
	//GraphWatched watched = new GraphWatched();
	// -----------------------------------parameters
	// private SketchLayer currentLayer;
	private SimpleInkObject currentStroke = null;
...
//	private long PrevcurrentStrokeTime = 0;
	protected boolean flagSelectOn=true;
...
//			 DragStrokeCount=0;
//			currentStroke = new SimpleInkObject();
//			data.PointData pd = new data.PointData();
...
//					oldy=y;
	...
	public void paint(Graphics g) {
		...
		if (flagSelectOn) {
		  if (flagBoxDraw)
			DrawingDebugUtils.drawRectangle((Graphics2D) g,box);
		}
		if (SystemSettings.DEBUG && Moving){
...
//
		...
		if(StrokeData!=null){
			logger.info("  in paing drawing the strokkesss....    ");
			StrokeData.paint((Graphics2D) g, this.getSize());
		}
...
//		watched.notifyObservers((Graphics2D) g);
	...
	public boolean isFlagSelectOn() {
		return flagSelectOn;
	}

	public void setFlagSelectOn(boolean flagSelectOn) {
		this.flagSelectOn = flagSelectOn;
	}
...
//					PrevcurrentStrokeTime = 0;
	...
	public boolean isFlagBoxDraw() {
		return flagBoxDraw;
	}

	public void setFlagBoxDraw(boolean flagBoxDraw) {
		this.flagBoxDraw = flagBoxDraw;
	}

	public 	GuiPageModel getStrokeData() {
		return StrokeData;
	}

	public void setStrokeData(	GuiPageModel strokeData) {
