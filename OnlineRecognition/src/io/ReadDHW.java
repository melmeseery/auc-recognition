...
private ArrayList<int[]> Stroke;
private ArrayList<ArrayList<int[]>> AllStrokes;

public ReadDHW() {
	org.apache.log4j.PropertyConfigurator.configure("log4j.properties");
}

public void readTheFile(String dir, String inFileName){
    AllStrokes = new ArrayList<ArrayList<int[]>>();
    Stroke = new ArrayList<int[]>();
    boolean debug = false;
    try {
        RandomAccessFile file = new RandomAccessFile(new File(dir + inFileName), "r");
        ...
        while ((next = file.read()) != -1) {
            if ((next & ~0x07) == 0x80) {//# pen up/down
                ...
                if ((next & 0x01) > 0) {
                    ...
                    pointcmd = "S";
                    if (debug) {
                        System.out.println("pen down POSITION " + count);
                    }
                 //   logger.info("pen down POSITION " + count);
                }
                else {
                    ...
                    strokes++;
                    if (debug) {
                        System.out.println("pen up POSITION " + count);
                    }
                 //   logger.info("pen up POSITION " + count);
                }
                color = ((next >> 1) + (layer * colorlayers)) & 0x03;
                if (debug) {
                    System.out.println("pen color " + color);
                }
               // logger.info("pen color " + color);
                if (color != lastcolor) {
                    lastcolor = color;
                }
            }
...
public ArrayList<ArrayList<int[]>> getStrokes() {
    return AllStrokes;
}
