...
for a=0:8
    for b=a+1:9
        ...
        for k=1:6
            ...
            set=training_set(find(or(labels==a,labels==b)),selection);
            binary_labels=labels;
            binary_labels(labels==a)=1;
            binary_labels(labels==b)=-1;
            binary_labels(~or(labels==a,labels==b))=[];
            current_set=[current_set; set];
            current_binary_labels=[current_binary_labels; binary_labels];  
        end
        fid= fopen(['ourfeatures_latin_training_set_fs_' int2str(a) 'vs' int2str(b) '.txt'],'wt');
