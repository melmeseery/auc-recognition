 ...
 *
 */
public class wekaClassifier extends GeneralClassifier implements Serializable {
 protected HashMap _labelToType = new HashMap();
	transient Instances data;
	 transient Instances Testdata;
	Instances unFilterData;
		String[] ClassifierOptions;
	transient	 weka.classifiers.Classifier classifier;
	  ...
	  transient Evaluation eval;
	  double [] SortedTP;
	  String [] SortedLabels;
		 
	private static transient final Logger logger = Logger.getLogger( wekaClassifier.class);
	transient Logger loggerWeka = Logger.getLogger("WekaApp");	
	transient private static final Logger logResult=Logger.getLogger("result");
...
//		 at classifiers.MultiFeature.ImageRecognizier.Test(ImageRecognizier.java:168)
   ...
   ////////////////////////////////////////////Setting the data features and attribust 
	...
	void setTrainSet(DataSet dr) {
             ...
             _labelToType=new HashMap<Integer, String>();
           
           // Declare the class attribute along with its values
           FastVector fvClassVal = new FastVector(categories.size());
           for (int i = 0; i < categories.size(); i++) {
        	   fvClassVal.addElement(""+categories.get(i));
          _labelToType.put(new Integer(i),""+categories.get(i));
		}
           Attribute ClassAttribute = new Attribute("Cat", fvClassVal);
       
           logger.info(" label to type is "+_labelToType);
...
//			logger.info("  info for the weka is "+iExample.attribute(1));
			...
			// noo ... Ineed a map here from the train set not the test set... 
			
			for (int i = 0; i < types.length; i++) {
				types[i]=mapIndex((int)i);
			}
		} catch (Exception e) {
	...
	private String mapIndex(int i) {
		String temp=(String)_labelToType.get(new Integer(i));
...
//			  }
	...
	public void loadModel(String string) {
		try{
	        ...
	        wekaClassifier  libClass= (  wekaClassifier ) is.readObject();
	        ...
	        this._labelToType= (HashMap) libClass._labelToType.clone();
	        this.type=libClass.type;
	        logger.info(" after read ...  label to type is "+_labelToType);
	        this.ClassifierOptions=libClass.ClassifierOptions;
	...
	public void saveModel(String string) {
		 ...
		 try {
		    ...
		    ObjectOutput os = new ObjectOutputStream(new FileOutputStream(string+".ser"));
	        os.writeObject(this);
...
//			}
			...
			if (i%5000==0)
		    logger.info("Sample "+i+" target is "+target+"  label target is  "+labeltarget+"  perdict is "+cat+"  the output of the classifiers is "+ distribution);
		...
		logger.info("    the result of classifier iss         "+percentC+"    sample "+SamplesCount);
