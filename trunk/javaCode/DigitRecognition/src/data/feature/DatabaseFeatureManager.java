...
public class DatabaseFeatureManager {
	...
	DataBaseConnector  db=new ArabicDataBaseConnector ();
	...
	public void setStartD(int d){
		...
		((ArabicDataBaseConnector)  db).setStartD(d);
	}   
	public void compteFeaturesToTrainFileAllDigit(){
		...
		//	db.Status=db.TEST;
			...
			/// application 
			ArrayList<String> filenames =	((ArabicDataBaseConnector) db).getImageFilesForAll();
			/// here we get all filename from the 
			if (db.getStatus()!=ArabicDataBaseConnector.TEST)
				filenames =lib.shuffleFiles(filenames);
		    ...
		    if (db.getStatus()==ArabicDataBaseConnector.TRAIN)
			...
			datastring=""+(Digit.getComputedFeaturesCount()+1);
			output.write(datastring);
			output.newLine();
		...
		for (int j = 0; j < filenames.size(); j++) {
			...
			imreader.setFile(filenames.get(j));
...
//	}
	 ...
	 * 
	 */
	public void generateStateForAllDigits(){
		
		
	//	int minf,maxf;
		
		//for every digit. 
		for (int d =	((ArabicDataBaseConnector)db).getStartD(); d <  AppDefaults.CATEGORY_SIZE; d++) {
		
			ArrayList<String> filenames = 	((ArabicDataBaseConnector)  db).getImageFilesForDigit(d);
			...
			for (int j = 0; j < filenames.size(); j++) {
				...
				for (int k = 0; k < count; k++) {
					...
					featuresValues[k][j]=image.getFeatures().get(k).getValue();
...
//					}
	...
	public void initFeaturesStat(){
		...
		int count =Digit.getComputedFeaturesCount();
	
		for (int i = 0; i <count; i++) {
			 tempfeat =new FeatureStat(Digit.getFeatureName(i));
	...
	public void setDb(DataBaseConnector dbx) {
		this.db = dbx;
	}	
