		...
		for (int i = 0; i < columeProfile.length; i++) {
			...
			if (i<half_w){
				if (columeProfile[i].begin>0){
					int tempsum=0;
					for (int j = columeProfile[i].begin; j < columeProfile[i].end; j++) {
						if (digitImage[j][i]==WHITE){
						tempsum++;
						}
...
//			}// / only in the fist half width...
		...
		// lxce=lxce/lre(H);---
		 ...
		 //lxce = (double)lxce / (double) rowProfile[rowProfile.length - 1].end;
		 ///  
lxce/=width;
if (countW!=0){
		mce69 /= (double) countW;
...
//					continue;
	...
	// locationOflxrb=lxrb;
		...
		int[] le12 = new int[sizele12-1];
		int k=0;
		for (int i = lne; i < rowProfile.length-1  && k < le12.length; i++) {
			// le1=lre(lne:H_1-1);
			// le2=lre(lne+1:H_1);
			// le12=le2-le1;
			if ((i + 1) < rowProfile.length){
				le12[k] = rowProfile[i + 1].end - rowProfile[i].end;
				k++;
			}
		}
		snv = 0.0;
