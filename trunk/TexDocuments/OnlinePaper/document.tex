...
%First, the issue of the names ”Arabic digits” and ”Latin digits” has to be resolved because naming conventions for different numeral systems may be confusing. Digits used in Europe and several other countries are sometimes called “Arabic Numbers”; and digits used in the Arab world are sometimes called ”Hindi Numbers”. Unlike online Arabic digits, the research in online Hindi digits is very limited. In this paper, we use a different naming convention. Digits used in the Arab world will be referred to as”Arabic Digits” and those digits used in Europe will be referred to as  ”Latin Digits” .
...
% Even though Arabic handwriting has recently gained a lot of attention \cite{ICDAR2009,Hamdani2009,ARKherallah2009}, 
%the problem of on-line Arabic digits has been neglected for quite some time now.
% Most systems working on Arabic handwriting either focus on off-line Arabic character and digit recognition problems or 
%only on-line Arabic character
% recognition problem \cite{ARAlamri2008,ARMargner2006,Mezghani2002,Mezghani3332005}. 
%Only a few Persian and Urdu digit (which are similar to Arabic digits) were presented \cite{razzak2009,Potrus2010} but
% they do not an efficient solution to the recognition problem. Specialized on-line Arabic digits systems are extremely rare or non-existing.   
% It has to be noted that the results of different works cannot be compared
% because the used databases do not have the same sizes or formats. A system for Online Latin
% digits recognition is presented by Kherallah et.al (2008)  \cite{Kherallah2008}.
% This system created an elliptical beta model for input digits and then compared
% it with stored digit models. The system reported results 95.08\% using MLPNN
% (Multi layer perception neural network) and SOM (Self organizing maps).  It
% should be noted that they gathered a 30,000 samples from 30 users to use in
% training and testing the system. The result of system by Razzak et.al
% \cite{razzak2009} is 97.4\% which was tested on 900 samples for both Urdu and
% Arabic numerals.One of the few recent systems on Arabic digits was presented by
% Portus et.al (2010) \cite{Potrus2010}, their system focused on stroke normalization and rectification process. Portus \cite{Potrus2010} generates a DNA sequence from a freeman code of the rectified strokes. A partial and global alignment algorithm is used to match DNA sequence extracted from the input strokes. The result of the system is 97.83\% but it is.
 %The system uses structural features of the digits to extract features which are used in the recognition phase. %Both \cite{razzak2009} and \cite{Potrus2010} presents Arabic online recognition system but they clearly tested them on small test set compared to our 30000 samples.  


%    \begin{table*}[th!]
%  \begin{center}
% \caption[Different system results]{Table compares between Results of other systems}
% \scalebox{0.75}{
% \begin{tabular}{|c|p{5cm}|c|p{5cm}|}  
% \hline
% \textbf{System}&\textbf{ Methods }&\textbf{Accuracy}&\textbf{Notes} \\
% \hline  
%  & & &  \\ 
% Kherallah et.al (2008) \cite{Kherallah2008}  & Trajectory and velocity elliptical modeling for input strokes & 95.08\% & Latin digits system tested on 30,000 digits collected from 24 person. \\
% Razzak et.al (2009)\cite{razzak2009}  & Structure feature using hybrid ( HMM and fuzzy logic) recognition  & 97.4\% &  The Urdu digits are most similar to Arabic digits but not the same. \newline System tested on 900 samples from 30 trained users.  \\
% Portus et.al (2010) \cite{Potrus2010}& Stroke normalization and rectification then uses freeman code to generate DNA sequence. A partial and global alignment algorithm is used to match DNA sequence extracted from the input strokes. & 97.83\% & System tested on 75 samples per digit.  \\
% \hline
% %  	Features	& Accuracy \\ \hline
% %     Offline	 &    97.48\%\\   \hline
% %  	Online	& 98.62 \\ \hline
% %  	Online and Offline & 98.73\%  \\ \hline  razzak2009,Potrus2010,
% %
% \end{tabular}
% }
% \label{tab:ResultsSystems}
% \end{center}
% \end{table*}


%% here add apart the diffference with hindi and latin and arabic digits. 


This paper aims at filling this void in the literature. The first contribution of this paper is presenting an Arabic on-line digits dataset collected form 300 writers. The second main contribution of this paper is introducing an efficient system to recognize on-line Arabic digits. The system is divided into a a pre-processing stage that eliminates the noise coming from the input device. After pre-processing, the features extraction stage computes a set of both on-line and off-line features from the users' strokes. The final stage uses the computed feature vector to classify the input into one of the Arabic digits. %Table \ref{tab:arabicandlatin} shows Arabic .
...
\begin{figure}[h]
\centering
 \subfigure['W2']{\label{fig:fig3}\includegraphics[width=0.04\textwidth]{figures/fig3.jpg}}
 \subfigure['W3']{\label{fig:fig4} \includegraphics[width=0.04\textwidth]{figures/fig4.jpg}}
\subfigure['W4'.]{\label{fig:fig5}\includegraphics[width=0.04\textwidth]{figures/fig5.jpg}}
  \subfigure['W5']{\label{fig:fig6}\includegraphics[width=0.05\textwidth]{figures/fig6.jpg}}
 \subfigure['W6]{\label{fig:fig7}\includegraphics[width=0.05\textwidth]{figures/fig7.jpg}}
   \subfigure['W7']{\label{fig:fig9b}\includegraphics[width=0.05\textwidth]{figures/fig9b.jpg}}
    \subfigure['W8]{\label{fig:fig9}\includegraphics[width=0.05\textwidth]{figures/fig10.jpg}}
     \subfigure['W9']{\label{fig:fig11b}\includegraphics[width=0.05\textwidth]{figures/fig11b.jpg}}
\subfigure['B69']{\label{fig:fig11b}\includegraphics[width=0.05\textwidth]{figures/fig12b.jpg}}
...
\begin{figure}
	\centering
	  \subfigure[Arabic '0' Versus Other digits]
	  {\label{fig:zeroVersusOther}\includegraphics[scale=0.17]{Zero1}}
	    \subfigure[Samples of the Arabic Digit]   {
	    \label{fig:zerosize}\includegraphics[scale=0.17]{Zero0}}
...
%  \begin{figure}
% 	\centering
% 		\includegraphics[scale=0.17]{Zero0}
...
%  \begin{figure}
% 	\centering
% 		\includegraphics[scale=0.17]{Zero1}
...
\centering
  ...
  \subfigure{\label{fig:figpaper}\includegraphics[width=0.25\textwidth]{page.JPG}}
  %\subfigure[Feature 'White\_3'.]{\label{fig:fig8}\includegraphics[width=0.15\textwidth]{figures/fig8.jpg}}

    \subfigure{\label{fig:scriptresult}\includegraphics[width=0.5\textwidth]{res.JPG}}
