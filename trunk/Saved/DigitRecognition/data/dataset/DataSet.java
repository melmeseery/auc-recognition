...
public class DataSet extends Observable implements Cloneable , Serializable {
	...
	ArrayList<Integer> ClassSamples;  // count of the number of sample in each categories 
...
//			
   ...
   public DataSet GenerateDataSetFromSettings(ClassifierData data2) {
	...
	public DataSet GenerateSplitClassesDataSet(ArrayList<Integer> c1, ArrayList<Integer> c2){
		...
		logger.info("  I am changing the data set into binarry data set with 0 mand 1");
			...
			//		if (s%100==0)
			//		logger.info(c1+"  changing  "+"  sample  ("+s +") "+dataTargets[s]+"   int  "+1);
					...
					//if (s%2000==0)
					//logger.info(c2+"  changing  "+"  sample  ("+s +") "+dataTargets[s]+"   int  "+11);
...
//	}
	...
	private void fixClassArray(){
		...
		for (int i = 0; i < ClassSamples.size(); i++) {
			logger.trace("  the label "+ClassValues.get(i)+"  has "+ClassSamples.get(i)+"  sample ");
		}
	...
	public void SaveToFile(String filename){
		...
		if (format==FILE_INPUT_FORMAT_EXCELL){
			if (this.ValidationCreated){
			  ...
			  String ValidationFile=filename.replace(".xls", "_validation.xls");
		  ...
		  SaveToTorchFileValidationSet(ValidationFile);
			...
			if (this.ValidationCreated){
				...
				trainFile=filename.replace(".txt", "_train.txt");
			     ...
			     ValidationFile=filename.replace(".txt", "_validation.txt");
				  }
			     ...
			     ValidationFile=filename.replace(".arff", "_validation.arff");
				  }
				  else {
...
if ( SaveSampleOffset){	
		if (this.ValidationCreated){
		  ...
		  String ValidationFile=filename.replace(".txt", "_validation_offset.txt");
 ...
 void SaveToxlsFileValidationSet(String filename) {
        ...
        for (int i = 0; i < featemp.size(); i++) {
		  row.createCell((short)(i+1)).setCellValue( new HSSFRichTextString (featemp.get(i).getFeatureName()));
		}
		    
        row.createCell((short)(featemp.size()+1)).setCellValue( new HSSFRichTextString ("Classes"));
          ...
          for (int i = 0; i < this.NumOfSamples; i++) {
        	  ...
        	  if (UseStringLabels){
             	   row.createCell((short)this.NumOfFeatures).setCellValue(new HSSFRichTextString ( targetsString.get(i)));
        	  }
        	  else {
...
void SaveToxlsFile(String filename) {
     		...
     		//    row.createCell((short)0).setCellValue( new HSSFRichTextString ( "Features"));
                ...
                for (int i = 0; i < featemp.size(); i++) {
				  row.createCell((short)(i)).setCellValue( new HSSFRichTextString (featemp.get(i).getFeatureName()));
				}
     		    
                row.createCell((short)(featemp.size())).setCellValue( new HSSFRichTextString ("Classes"));
...
// String line;HW,vol2,Vol(1),Vol(2),w2r,wu,w4lft,w4,wrb,mxb,wd,u1,smallwd,sre,mce69,sr,im,wce,wcb,wre,btop,bbot,sle3,sle,snv,cbe,wcbe,szero,mxlre6,lxre6,lxrb,mxce,lxce,blft,bright]
			try {
...
// reading the @attrib....
			...
			FeatureStat feat3;
	          ...
	          while(input.hasNext()& ! dataFound){
	        		  ...
	        		  if (temp.trim().startsWith("@attrib")){
                         ...
                         if (temp.contains("class")){
                        	 ...
                        	 for (int i = 0; i < classesnames.length; i++) {
								...
								ClassLabels.add(classesnames[i]);
                        			 ...
                        			 logger.info("EEEEEEEEEEEEEEEEEE --- not number targetssss  s" );
                        			   ...
                        			   if(_labelToType.containsKey(classesnames[i])){
		        			 ...
		        			 String t;
	                        	...
	                        	// logger.info(" trying to find a ' in the "+s+"  to " +e  );
	                        	 t=temp.substring(s+1, e);
					    	  tempF=t.trim();
					    	  indexF=F;
					    	  F++;
					    	  feat3=new FeatureStat();
			  	...
			  	features.setAllFeaturesRead(FeatureStatList);
			  	logger.info("number of features got after  reading "+features.getAllFeaturesRead().size());
...
//			      
			     ...
			     logger.info("  total number of lines read  =  "+linecount+"  each has "+NumOfFeatures+"  feature");
...
1   , 129 , 0 , 1 , 1 , 0 , 6 , 0 , 0 , 7.33333 , 14.666
          ...
          file = new FileOutputStream(filename);
                 ...
                 out = new PrintStream(file);
                  ...
                  for (int i = 0; i < featemp.size(); i++) {
                	   ...
                	   out.println(" @attribute   '"+featemp.get(i).getFeatureName()+"'  numeric  ");
                 ...
                 // out.println ( this.NumOfSamples +" "+ (this.NumOfFeatures+1));
                  double dataw;
                  logger.info("Start writing ............................"+this.NumOfSamples+"  samples");
                  for (int i = 0; i < this.NumOfSamples; i++) {
      ...
      out.close();
      logger.info("  total number of lines read  =  "+ this.NumOfSamples+"  each has "+NumOfFeatures+"  feature");
          }
          catch (Exception e){
                  System.err.println ("Error in writing to file");
...
//	      }
	...
	public ArrayList<FeatureStat>  getComputeFeatureState() {
		ArrayList<FeatureStat> fstate=new ArrayList<FeatureStat>(features.getAllFeaturesRead());
...
//		} 
	...
	/**
	 * @param orignalLocations the orignalLocations to set
	 */
	...
	public void compareDataSet(DataSet dset){
		...
		if (dset.NumOfSamples!=NumOfSamples){
			...
			logger.info(" number of sample is different but will work on min which is  "+minS);
		}
		...
		minF=NumOfFeatures;	
		 ...
		 if (dset.NumOfFeatures!=NumOfFeatures){
				...
				logger.info(" Number of features is different but will work on min which is "+minF);
			}
		 ...
		 for (int i = 0; i < minS; i++) {
			 ...
			 for (int j = 0; j < minF; j++) {
				 if (!tsample[j].equals(sample[j])){
					 logger.info("  There is features "+j+"  not equal in both data set in Sample  "+i +"   first data value is "+sample[j]+"  and the second is " + tsample[j]);
					 errors++;
					 break;
				 }
			}
