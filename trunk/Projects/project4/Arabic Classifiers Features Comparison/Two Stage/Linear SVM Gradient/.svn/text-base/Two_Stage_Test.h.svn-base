...
{	
	...
	DiskXFile linear_svm_model_file("zc_linear_svm_model", "r");
	...
	SVMClassification *lin_svms[10][10];
	float s=1;
	Kernel *lin_kernel = new(allocator) DotKernel(s);
	...
	for(int i=0; i<=8; i++)
	{
		for(int j=i+1; j<=9; j++)
		{
			lin_svms[i][j] = new(allocator) SVMClassification(lin_kernel);

			lin_svms[i][j]->loadXFile(&linear_svm_model_file);

			bs[count]= lin_svms[i][j]->b;

			for(int n=0; n<num_of_features; n++)
				Ws.frames[count][n]=0;

			for(int s=0; s<lin_svms[i][j]->n_support_vectors; s++)
			{
				for(int n=0; n<num_of_features; n++)
					Ws.frames[count][n]+= (lin_svms[i][j]->sv_alpha[s])*(lin_svms[i][j]->sv_sequences[s]->frames[0][n]);
			}
	...
	float accuracy(0);
	float thresh=0.903808;//0.903808;
	int K=6;
