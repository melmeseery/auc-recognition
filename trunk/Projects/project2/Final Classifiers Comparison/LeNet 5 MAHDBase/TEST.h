...
void TEST()
{
	...
	//char test_file[]="D:/datasets/mahdbase32by32_testing_set.txt";
char test_file[]=test_MAHD_32_file;  //"D:/datasets/mahdbase32by32_testing_set.txt";
	...
	bool found(false);
	for(int t=0; t<Nt; t++)
		if(correct[t]/(MaxTestSize-n_rejected[t])*100>99.5)
		{
			fout<<"Accuracy without rejection: "<<actual_accuracy/100<<"%"<<endl;
			cout<<"Accuracy without rejection: "<<actual_accuracy/100<<"%"<<endl;

			fout<<"Accuracy after rejection: "<<correct[t]/(MaxTestSize-n_rejected[t])*100<<"%"<<endl;
			cout<<"Accuracy after rejection: "<<correct[t]/(MaxTestSize-n_rejected[t])*100<<"%"<<endl;
