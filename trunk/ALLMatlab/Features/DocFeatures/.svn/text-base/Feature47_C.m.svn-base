function  [featureSet,AverageFeatureTime,Totaltime,AvergeTotaltime]=Feature47_C(A,labels,digit)
...
start_of_row=17+784*(places(k)-1);
        ...
        W_1=W-1;
        H_1=H-1;
        rb2=rb-1;cb2=cb-1;H2=fix(H/2);H3=fix(2*H/3);W2=fix(W/2);H1=fix(H/3);
        H4=fix(3*H/4);
        I=I.*m28;
%--------------------------------------------------------------------------
area=H*W;
...
ic=[];
 ...
 if (clrb)
        ...
        dlrb=lrb(i2+1)-lrb(i2);
        if (dlrb)
            wabv=wabv/dlrb;
        end    
 end   
    if (wabv>1.7&i2<10&i2-rx4>1)
        fl74=1;
    end%44 7s and 3 4s
    if(wabv>4&i2<=13)
        fl74=1;
    end   
    if (i2<H&rx4<=H1&i2-rx4>4)
        fl74=1;
    end 
%--------------------------------------------------------------------------
 
 times(n)=toc(startTime);
 features = [rx4,btop,mix,slc,wtop,slrbe,alc,i2,wabv,fl74];
 featureSet(k,:)=features;
end

%compute time 
Totaltime=toc(featureTime)
AvergeTotaltime=Totaltime/LP
AverageFeatureTime=sum(times)/length(times)
