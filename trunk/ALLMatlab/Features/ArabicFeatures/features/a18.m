function [featureSet]=a18(A,Base,digit)
%digit=7;
c1=0;c2=0;
th=20;
%--------------------------------------------
...
%--------------------------------------------------------------------------
wd=0;fl1=0;im=0;hb=H-H2+1;fl8=0;w2r=0;w4=0;wcbe=0;wu=0;
for i=1:H
    for j=W:-1:1
        if (~m(i,j))
            if(j>lrb(i)&j<lre(i)&i>lce(j))wd=wd+1;im=i;
            elseif(i<lcb(j)&j>lrb(i)&j<lre(i))wu=wu+1;
            end
        end   
    end
end
...
if(wd<3)fl1=1;
elseif(wd<15&im<H2)fl1=1;
elseif(HW>=3.4)fl1=1;
end    

%if (~fl1&~fl8)
%    c2=c2+1;
%    I,disp=[HW,wd,k],keyboard,end
%end    

if (fl1)c1=c1+1;else c2=c2+1;end
% one mistake in 1 (k=636) slightly looks like 8
%--------------------------------------------------------------------------
Features=[wd,im-H2,HW];
