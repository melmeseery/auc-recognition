function  [predict_label, accuracy, Finaldec_values,conf]=OVOTestMatlab(groups, dataTest, svmStruct, CatSize)
 ...
 for s=1:length(dataTest)
      ...
      if(mod(s,1000)==0) 
        ...
        r=zeros(CatSize,CatSize);
  for i=1: CatSize
      for j=i+1: CatSize
          
            [labelData , dec]= svmclassify2(svmStruct(i,j),dataTest(s,:));
            accur=1; 
           % dec=labelData; 
          %  [predict_label, accuracy, dec_values] = svmpredict(heart_scale_label, heart_scale_inst, model);
           ...
           r(i,j)=1.0/(1.0+exp(dec));% if it is 
           r(j,i)=1.0/(1.0+exp(-dec));%%
      end  
  end  
  ...
  %%% get the decsioion of each class seperate.... 
    for l=1: CatSize
      ...
      for k=1: CatSize
          ...
          sumrij=sumrij+(1/r(l,k)); 
          if ( decvalue(l,k)~=0)
          % other wise 
          sumtemp=sumtemp + ((1/decvalue(l,k))- CCKK);
        % sumtemp=sumtemp + ((1/decvalue(l,k)));
        
          end 
      end
      prob(l)=1/(sumrij-CCKK);
      decValueClass(l)=1/(sumtemp-CCKK);
    end 
           [ma mi]=max(vote);
              classes(s)= mi;
              predict_label(s)=classes(s);
              %%decValueClass(predict_label(s))
              %Finaldec_values(s)= decValueClass(predict_label(s)); 
              Finaldec_values(s,:)=prob;
