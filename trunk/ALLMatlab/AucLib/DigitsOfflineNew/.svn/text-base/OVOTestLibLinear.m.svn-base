function  [predict_label, accuracy, dec_values,conf]=OVOTestLibLinear(groups, dataTest, svmStruct, CatSize)
  ...
  for i=1: CatSize
      for j=i+1: CatSize
          ...
          %  [predict_label, accuracy, dec_values] = svmpredict(heart_scale_label, heart_scale_inst, model);
            [labelData, accur, dec] =predict(groups(s),sparse(dataTest(s,:)), svmStruct(i,j));
            decvalue(i,j)=max(dec); 
            if(labelData>0)
                vote(i)=    vote(i)+1;
            else
                 vote(j)=    vote(j)+1;
