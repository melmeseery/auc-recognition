...
if (PrePareData)
    ...
    if(RecomputeFeatures==1)
         ...
         data_Hmm=getHmmOberservations(Database,ReadImages,isOnline,'Feature',Feature,'NumberOfSplits',NumberOfSplits,'OverlapSize',OverlapSize,'NumberOfZones',NumberOfZones);
...
if (train)
    ...
    %[A,B,pi1,loglike_m] = hmmtrain2(data,States,maxiter,tol,cb,R,A_m,B_m,pi_m); % 4: states 16 : observations
     [A,B,pi1,loglike_m] = hmmtrain2(data,States,maxiter,tol,cb,R,A_m,B_m,pi_m,isOnline,'Feature',Feature,'NumberOfSplits',NumberOfSplits,'OverlapSize',OverlapSize,'NumberOfZones',NumberOfZones); % 4: states 16 : observations
...
if (test)
    ...
    for i= 1 :SizeCategories
        ...
        %[logp,guess{i}] = hmmrecog( dataTest{i},A, B, pi1,cb);
         [logp,guess{i}] = hmmrecog( dataTest{i},A, B, pi1,cb,isOnline,'Feature',Feature,'NumberOfSplits',NumberOfSplits,'OverlapSize',OverlapSize,'NumberOfZones',NumberOfZones);
