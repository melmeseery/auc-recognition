function [A_m,B_m,pi_m,loglike_m] = hmmtrain2(data,S,maxiter,tol,cb,R,Ai_m,Bi_m, pi_m ,varargin)
...
%     s = load(file_s);                % Load word signal from file.
    ...
    y = hmmfeatures(s,varargin{:}); % Extract feature vectors.
...
i
   [A,B,pi1,loglike] = hmmfb(ytrain,S,K,maxiter,tol);  % when using Vertibi
