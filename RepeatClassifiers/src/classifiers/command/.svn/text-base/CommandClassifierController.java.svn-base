...
public void setParam(String param) {
	...
	//new type
		lib_type=type;
		if (lib_type==TaskSettings.TASK_LIB_LIBSVM){
			classifier=new LibSvmCommandClassifier();
			
		}
		else  if (lib_type==TaskSettings.TASK_LIB_TORCH)
		{
	...
	private String CreateTorchOptions(){
		...
		String op="";
		algorithmString="";
		if (alg==TaskSettings.TASK_ALG_SVM)
		{
		...
		if (alg==TaskSettings.TASK_ALG_NEURAL)
		{
			 op+=" -a 6";
